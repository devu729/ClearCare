...
export default function AppealDrafter() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Pre-fill patient name if provided
  ...
  // Replace placeholders in letter with filled values
  const fillPlaceholders = () => {
    ...
    Object.entries(replacements).forEach(([placeholder, value]) => {
      if (value) filled = filled.replaceAll(placeholder, value)
    })
  ...
  const fieldDef = [
    { key: 'date',            label: 'Letter Date',        placeholder: 'e.g. March 22, 2026' },
    ...
    { key: 'provider_name',   label: 'Provider / Doctor Name', placeholder: 'e.g. Dr. Sarah Johnson' },
    { key: 'provider_address',label: 'Provider Address',   placeholder: 'e.g. 123 Medical Ave, Houston, TX 77001' },
    { key: 'provider_phone',  label: 'Provider Phone',     placeholder: 'e.g. (713) 555-0100' },
    { key: 'insurance_address',label:'Insurance Company Address', placeholder: 'e.g. P.O. Box 272540, Chico, CA 95927' },
  ]
  ...
  return (
    <div className="ad-root">
      <style>{`
        ...
        .ad-btn {
          padding: 11px 28px; background: linear-gradient(135deg, #0284c7, #0ea5e9);
          border: none; border-radius: 8px; color: #fff;
          font-family: 'Outfit', sans-serif; font-size: 14px; font-weight: 600;
          cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(14,165,233,0.25);
        }
        .ad-btn:hover:not(:disabled) { transform: translateY(-1px); }
        .ad-btn:disabled { opacity: 0.45; cursor: not-allowed; transform: none; }
        .ad-btn-ghost {
          padding: 9px 16px; background: #fff; border: 1.5px solid #bae6fd; border-radius: 8px;
          color: #0369a1; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 500;
        ...
        .ad-btn-warning {
          padding: 9px 16px; background: #fff7ed; border: 1.5px solid #fed7aa; border-radius: 8px;
          color: #c2410c; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 500;
          cursor: pointer; transition: all 0.2s; white-space: nowrap;
        }
        .ad-btn-warning:hover { background: #ffedd5; }

        .ad-loading {
          display: flex; align-items: center; gap: 10px; padding: 16px;
          background: #f0f9ff; border-radius: 10px; font-size: 13px;
          color: #0369a1; margin-bottom: 16px;
        }
        .ad-spinner {
        ...
        .ad-error {
          ...
          border-radius: 8px; font-size: 13px; color: #dc2626; margin-bottom: 16px;
        }
        ...
        .ad-letter-header {
          background: #f0f9ff; padding: 14px 20px; border-bottom: 1px solid #bae6fd;
        ...
        /* Unfilled warning badge */
        .ad-unfilled-badge {
          display: inline-flex; align-items: center; gap: 5px;
          background: #fff7ed; border: 1px solid #fed7aa; border-radius: 6px;
          padding: 5px 10px; font-size: 11px; color: #c2410c; font-weight: 600;
        }
        ...
        .ad-fill-input {
          ...
          font-family: 'Outfit', sans-serif; font-size: 13px; color: #0f172a;
        ...
        .ad-fill-apply {
          padding: 9px 20px; background: linear-gradient(135deg, #d97706, #f59e0b);
          border: none; border-radius: 8px; color: #fff;
          font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 600;
          cursor: pointer; transition: all 0.2s;
        }
        .ad-fill-apply:hover { transform: translateY(-1px); }
        .ad-fill-cancel {
          padding: 9px 16px; background: #fff; border: 1.5px solid #e2e8f0;
          border-radius: 8px; color: #64748b; font-family: 'Outfit', sans-serif;
          font-size: 13px; cursor: pointer;
        }

        /* Letter body */
        .ad-letter-body {
          padding: 24px; font-family: 'DM Mono', monospace; font-size: 12.5px;
          color: #334155; line-height: 1.9; white-space: pre-wrap;
          max-height: 420px; overflow-y: auto; background: #fafeff;
        }
        .ad-letter-editable {
          padding: 24px; font-family: 'DM Mono', monospace; font-size: 12.5px;
          color: #334155; line-height: 1.9;
          width: 100%; min-height: 420px; border: none; outline: none;
          background: #fafeff; resize: vertical;
        }

        /* Highlight unfilled placeholders */
        .ad-letter-body mark {
          background: #fef9c3; color: #854d0e; border-radius: 3px;
          padding: 0 2px; font-weight: 600;
        }

        /* Send section */
        .ad-send-section {
          padding: 20px 24px; border-top: 1px solid #e0f2fe; background: #f8fafc;
        }
        .ad-send-title {
        ...
        .ad-deadline-badge {
          ...
          padding: 8px 14px; font-size: 13px; color: #c2410c; font-weight: 500;
        }
        .ad-ics-hint {
          font-size: 11px; color: #64748b; margin-top: 8px;
          padding: 10px 14px; background: #f0f9ff; border-radius: 8px;
          border: 1px solid #e0f2fe; line-height: 1.7;
        }
        ...
        @media (max-width: 600px) {
          .ad-fill-grid { grid-template-columns: 1fr; }
        }
      `}</style>

      <h2 className="ad-heading">Appeal Drafter</h2>
      <p className="ad-sub">Generate a formal appeal letter — then fill in your details before sending</p>

      {/* ── Generation form ── */}
      <div className="ad-form">
        <label className="ad-label">Patient name (optional — can fill in later)</label>
        ...
        <textarea className="ad-input" rows={4}
          placeholder="Paste the denial reason, denial code, or full denial letter here..."
          value={denialText} onChange={e => setDenialText(e.target.value)} />

        <button className="ad-btn" onClick={generate}
      ...
      {loading && (
        <div className="ad-loading">
          ...
          Tracing denial rule → drafting arguments → writing letter...
        </div>
      )}
      ...
      {editedLetter && (
        <div className="ad-letter-card">
          ...
          <div className="ad-letter-header">
            <span className="ad-letter-title">✉️ Appeal Letter</span>
            <div className="ad-header-actions">
              {unfilledCount > 0 && (
                <span className="ad-unfilled-badge">
                  ⚠ {unfilledCount} placeholder{unfilledCount > 1 ? 's' : ''} to fill
                </span>
              )}
              <button className="ad-btn-warning"
                onClick={() => setIsEditing(!isEditing)}>
                {isEditing ? '👁 Preview' : '✏️ Fill Details'}
              </button>
              <button className={`ad-btn-ghost ${copyDone ? 'ad-btn-success' : ''}`}
          ...
          {isEditing && (
            <div className="ad-fill-form">
              ...
              <div className="ad-fill-sub">
                These replace the [PLACEHOLDERS] in your letter automatically.
              </div>
              <div className="ad-fill-grid">
                {fieldDef.map(f => (
                  <div className="ad-fill-field" key={f.key}>
                    <label className="ad-fill-label">{f.label}</label>
                    <input className="ad-fill-input" type="text"
              ...
              <div className="ad-fill-actions">
                ...
                <button className="ad-fill-apply" onClick={fillPlaceholders}>
                  ✓ Apply to Letter
                </button>
              </div>
            </div>
          ...
          {/* Letter body — editable textarea or preview */}
          {isEditing ? (
            <textarea
              className="ad-letter-editable"
              value={editedLetter}
              onChange={e => setEditedLetter(e.target.value)}
            />
          ) : (
            <div className="ad-letter-body">
              {editedLetter}
            </div>
          )}
          ...
          <div className="ad-send-section">
            ...
            {unfilledCount > 0 && (
              <div style={{
                ...
                ⚠ Your letter still has <strong>{unfilledCount} unfilled placeholder{unfilledCount > 1 ? 's' : ''}</strong>.
                Click <strong>"✏️ Fill Details"</strong> above to replace them before sending.
              </div>
            )}
            ...
            <div className="ad-send-row">
              <input className="ad-send-input" type="email"
                placeholder="Send to patient or clinician email..."
                value={email} onChange={e => setEmail(e.target.value)} />
              <button className={`ad-btn-ghost ${emailSent ? 'ad-btn-success' : ''}`}
                onClick={sendEmail}>
            ...
            {deadlineDate && (
              <div>
                <div className="ad-send-row">
                  ...
                  <button className={`ad-btn-ghost ${calSaved ? 'ad-btn-success' : ''}`}
                ...
                {!calSaved && (
                  <div className="ad-ics-hint">
                    📎 Downloads a <strong>.ics</strong> file — open it to add this deadline to
                    <strong> Google Calendar</strong>, Outlook, or Apple Calendar. No login required.
                  </div>
                )}
                {calSaved && (
                  <div className="ad-ics-hint success">
                    ✅ Open the downloaded <strong>.ics</strong> file to add the appeal deadline to your calendar.
                    Reminders set for 7 days and 1 day before.
                  </div>
                )}
              </div>
            )}
